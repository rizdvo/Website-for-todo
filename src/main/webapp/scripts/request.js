...
function edit_task(task_id) {
    ...
    let save_tag = "<button class='btn btn-success' id='save_" + task_id + "'>Save</button>";
    $(identifier_edit).replaceWith(save_tag);

    let save_identifier = "#save_" + task_id;

    let property_save_tag = "update_task(" + task_id + ")";
    $(save_identifier).attr("onclick", property_save_tag);

    let current_tr_element = $(save_identifier).parent().parent();
